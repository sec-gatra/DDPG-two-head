...
def main():
    ...
    if opt.render:
        st=0
        channel_gains_from_csv = np.load('channel_gains_from_csv.npy', allow_pickle=True)
        for i in range(len(channel_gains_from_csv)):
                            st+=1
                            #loc_eval= env.generate_positions() #lokasi untuk s_t
                            channel_gain_eval = channel_gains_from_csv[i]
